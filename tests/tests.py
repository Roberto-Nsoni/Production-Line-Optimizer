...
if len(sys.argv) < 4:
    print("Use: python3 tests.py <TEST_FILE> <OPT_FILE> <INSTANCES_DIR>")
    sys.exit(1)

TEST_FILE = sys.argv[1]  # Archivo que se quiera provar
OPT_FILE = sys.argv[2] # Archivo donde guardar resultados
INSTANCES_DIR = sys.argv[3]  # Carpeta con los archivos .txt de instancias


INSTANCES = sorted(glob.glob(os.path.join(INSTANCES_DIR, "*.txt")))
if not INSTANCES:
    print(f"[ERROR] There is no .txt files in {INSTANCES_DIR}")
    sys.exit(1)
...
def main():
    ...
    promedio = sum(total_time) / len(total_time)
    print(f"\n[TOTAL] Tiempo promedio: {promedio:.2f} s")
    return promedio
